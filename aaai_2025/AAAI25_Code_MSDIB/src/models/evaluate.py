...
def eval_experiment(cfg_name, tag, plot=False):
    ...
    for run in range(max_n_runs):
        ...
        if run_logs[f"metrics/acc"] > best_acc:
            best_acc = run_logs[f"metrics/acc"]
            best_logs = run_logs
            best_run = run
            best_net = net
...
def plot_representations(views, labels, net, project_method="tsne"):
    ...
    a[2] = th.nn.functional.pad(a[2], (0, 1000, 0, 0))
    ...
    plot_projection(X=fused, method=project_method, hue=fused_hue, ax=ax[1], title="ours epoch=120",
...
def evaluate_test(cfg_name, tag, plot):
    acc, nmi = eval_experiment(cfg_name, tag, plot=False)
    return acc, nmi
