...
Dataname = 'Reuters_dim10' # Reuters_dim10
...
if args.dataset == "Caltech-2V" or args.dataset == "MNIST-USPS" or args.dataset == "Hdigit" or args.dataset == "NUS-WIDE" or args.dataset == "Reuters_dim10":
    args.con_epochs = 110 
    seed = 200
...
for i in range(T):
    ...
    while epoch <= args.pre_epochs + args.con_epochs:
        ...
        acc, nmi, ari, pur ,y_pred , labels, H= valid(model, device, dataset, view, data_size, class_num, args=args, eval_h=False, epoch=epoch)     
    ...
    tsne = TSNE(n_components=2, 
                ...
                init='pca',
                random_state=i+1)
    feature_2d = tsne.fit_transform(best_H)
    ...
    plt.tight_layout()
    plt.savefig(f'{Dataname}_tsne_seed_{i+1}.png') 
