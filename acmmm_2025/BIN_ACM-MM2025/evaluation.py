...
def clustering(x, y):
    ...
    psedo_labels = km_torch.fit_predict(x)

    if np.min(y) == 1:
        y = y - 1
    ACC, NMI, ARI, Purity, Fscore, Precision, Recall, AMI = clusteringMetrics(y, psedo_labels.cpu().numpy())
    ACC = np.round(ACC, 4)
    NMI = np.round(NMI, 4)
    ARI = np.round(ARI, 4)
    ...
    scores =dict({'ACC': ACC, 'NMI': NMI, 'ARI': ARI,'Purity':Purity, 'F-score': Fscore,'Precision':Precision,'Recall':Recall,'AMI':AMI})

    ret = {}
    ret['kmeans'] = scores
    return ret, psedo_labels
