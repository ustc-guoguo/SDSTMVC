...
def load_mat(args):
    ...
    if args.dataset == "WIKI":
        ...
        mat = sio.loadmat(os.path.join(args.data_path, "BDGP.mat"))
        data_X.append(mat["X1"].astype("float32"))
        data_X.append(mat["X2"].astype("float32"))
        label_y = np.squeeze(mat['Y']).astype('int')

    elif args.dataset == "HandWritten":
        mat = sio.loadmat(os.path.join(args.data_path, "HandWritten.mat"))
        X = mat['X'][0]
        views = [0, 2]
        for view in views:
            data_X.append(X[view].astype('float32'))
        label_y = np.squeeze(mat['Y']).astype('int')
    ...
    elif args.dataset == "Reuters_dim10":
        ...
        views = [0, 1]
        for view in views:
            x = np.vstack((x_train[view], x_test[view])).astype('float32')
            data_X.append(x[idx])
        label_y = y[idx]
    
    elif args.dataset == "MNIST-USPS":
        mat = sio.loadmat(os.path.join(args.data_path, "MNIST.mat"))
        data_X.append(mat['X1'].astype('float32'))
        data_X.append(mat['X2']).astype('float32')
        label_y = np.squeeze(mat['Y']).astype('int')
    
    elif args.dataset == "DeepAnimal":
        mat = sio.loadmat(os.path.join(args.data_path, "DeepAnimal.mat"))
        data_X.append(mat['X'][0, 6].T.astype('float32'))
        data_X.append(mat['X'][0, 5].T.astype('float32'))
        label_y = np.squeeze(mat['gt']).astype('int')
    
    elif args.dataset == "Hdigit":
        mat = sio.loadmat(os.path.join(args.data_path, "Hdigit.mat"))
        X = mat['data'][0]
        data_X.append(X[1].T.astype('float32'))
        data_X.append(X[0].T.astype('float32'))
        label_y = np.squeeze(mat['truelabel'][0, 0]).astype('int')
