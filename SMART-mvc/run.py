...
def main(config):
    run = swanlab.init( 
        project="MVC-ACMmm2026",
        experiment_name=f"SMART_{config['dataset']}",
        group="减少训练轮次_100",
    ...
    logger = DualLogger(root=f"./logs/{config['dataset']}/{time}",
    ...
    if config['train']:
        seeds = [5, 15, 25, 35, 45] # [5, 15, 25, 35, 45]
    else:
        seeds = [5]
    for seed in seeds:
    ...
    swanlab.finish()
    print(f"{np.mean(scores['acc']) * 100:.2f} ± {np.std(scores['acc']) * 100:.2f}" + "｜" + 
          f"{np.mean(scores['nmi']) * 100:.2f} ± {np.std(scores['nmi']) * 100:.2f}" + "｜" + 
          f"{np.mean(scores['ari']) * 100:.2f} ± {np.std(scores['ari']) * 100:.2f}" + "｜" + 
          f"{np.mean(scores['pur']) * 100:.2f} ± {np.std(scores['pur']) * 100:.2f}") 
