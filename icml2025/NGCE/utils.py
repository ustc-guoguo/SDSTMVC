...
def batch_contrastive_lossv2(f_x0, f_x_1, mm):
    pos_similarity = torch.sum(f_x0 * f_x_1, dim=1)  
    neg_similarity_sum = torch.zeros_like(pos_similarity)  
    for i in range(mm):
        indices = torch.randperm(f_x_1.size(0))
        f_x_1_shuffled = f_x_1[indices]
        neg_similarity = torch.sum(f_x0 * f_x_1_shuffled, dim=1)  
        neg_similarity_sum += torch.exp(neg_similarity)  
    pos_exp = torch.exp(pos_similarity) 
    loss = -torch.mean(torch.log(pos_exp / (pos_exp + neg_similarity_sum)))  
    return loss

def batch_contrastive_lossv1(f_x0, f_x_1, f_x_2list):
    pos_similarity = torch.sum(f_x0 * f_x_1, dim=1) 
    neg_similarity_sum = torch.zeros_like(pos_similarity) 
    for f_x_2 in f_x_2list:
        neg_similarity = torch.sum(f_x0 * f_x_2, dim=1)  
        neg_similarity_sum += torch.exp(neg_similarity)  
    pos_exp = torch.exp(pos_similarity)  
    loss = -torch.mean(torch.log(pos_exp / (pos_exp + neg_similarity_sum)))  
    return loss
