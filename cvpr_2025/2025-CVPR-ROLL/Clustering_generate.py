...
def Clustering_generate(x_list, y):

    n_clusters = np.size(np.unique(y))
    x_final_concat = np.concatenate(x_list[:], axis=0)   #2n*d

    kmeans = KMeans(n_clusters)
    kmeans.fit(x_final_concat)
    cluster_centers = kmeans.cluster_centers_  # 输出聚类中心
    # print(cluster_centers.shape)
    # exit()


    kmeans = KMeans(n_clusters, init=cluster_centers, n_init=1)  # n_init=1 表示只运行一次以使用指定的初始聚类中心
    kmeans.fit(x_list[0])
    labels_tmp0 = kmeans.labels_
    
    # encoder = OneHotEncoder(sparse=False, n_clusters)
    # cluster_labels0 = encoder.fit_transform(labels_tmp0.reshape(-1, 1))
    
    cluster_labels0 = ind2vec(labels_tmp0.reshape([-1,1])).astype('int16') 


    kmeans = KMeans(n_clusters, init=cluster_centers, n_init=1) 
    kmeans.fit(x_list[1])
    labels_tmp1 = kmeans.labels_
    # encoder = OneHotEncoder(sparse=False, categories=[range(n_clusters)])
    # cluster_labels1 = encoder.fit_transform(labels_tmp1.reshape(-1, 1))
    cluster_labels1 = ind2vec(labels_tmp1.reshape([-1,1])).astype('int16') 





    return cluster_centers, cluster_labels0, cluster_labels1
