...
def main():
    ...
    parser.add_argument('--dataset_name', type=str, default='BDGP', help='The dataset used for training/testing') # MNIST-USPS & Hdigit & NUS-WIDE & Reuters_dim10
    parser.add_argument('--KAPPA', type=float, default=0.1, help='The hyperparameter for computing q')
    ...
    parser.add_argument('--hub_k', type=int, default=1, help='k for hubness calculation')
    ...
    parser.add_argument('--seed', type=int, default=17204, help='The seed of randomness.')
    ...
    for i in range(args.n_repeated):
        ...
        print(len(hub_VIEW_list)) # 2
        print(hub_VIEW_list[0])
        print(len(init_y_pred))
        ...
        if ACC > best_acc:
            best_acc = ACC
            best_pred = init_y_pred
            best_feature = combined_feature.detach().cpu().numpy()
            best_hub_VIEW_list = hub_VIEW_list
    ...
    if best_feature is None:
        raise RuntimeError("No best_feature collected for t-SNE visualization. Please check training loop.")

    n_samples = int(best_feature.shape[0])
    if n_samples < 2:
        raise RuntimeError(f"t-SNE requires at least 2 samples, got n_samples={n_samples}.")
    ...
    for i in range(args.num_classes):
        cluster_data = feature_2d[best_pred == i]
        if cluster_data.size == 0:
            continue
        col = colors[i] if i < len(colors) else '#'+''.join(random.choice('0123456789ABCDEF') for _ in range(6))
        ax.scatter(
            cluster_data[:, 0],
            cluster_data[:, 1],
            label=f'Cluster {i + 1}',
    ...
    fig.savefig(f'./results/{args.dataset_name}/hubREP_clustering.png')
    plt.tight_layout()
    logger.info('------------------------------')
    logger.info
